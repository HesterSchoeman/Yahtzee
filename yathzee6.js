...
for (var i = 0; i < player1.length ; i++) {
    ...
    if (!player1[i].classList.contains("TL") && !player1[i].classList.contains("BS")) {
...
for (var i = 0; i < player2.length ; i++) {
    ...
    if (!player2[i].classList.contains("TL") && !player2[i].classList.contains("BS")) {
...
function nofakind(number) {
    ...
    for (var i = 0; i <= 5 - number; i++) {
        ...
        if(nofakind.length == 5 && number == 5) {
            ...
            return numbers.reduce( (accum, currentvalue ) => { return accum + currentvalue },0);
        } else if(i == 5 - number){
            return 0;
        }
    }
...
//     if (distinctarray.length == 2 && (repeat == 2 || repeat == 3)) { return 25 };
//     return 0;
// }
const fullhouse =() => {
    const distinct = (value, index, self) => { return self.indexOf(value) === index; };
...
function calculatescore() {
    let scoreboard = [ 
        ...
        {
            selector: $chance,
            total: numbers.reduce( (a, b)=> {return a + b},0)
        }
    ];
